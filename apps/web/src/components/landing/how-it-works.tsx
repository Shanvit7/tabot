import { WEB_TARGET_DEFS } from "@tabot/shared/share/targets";
import { type ComponentType, useEffect, useRef, useState } from "react";

// Icons live in ~/lib/share-targets, which pulls @lobehub/icons (~140 KiB) plus
// its vendor chunk. The names are icon-free in @tabot/shared/share/targets (a
// leaf module — the package barrel would drag in dexie + @openredaction/core),
// so paint the list immediately and attach the brand icons once they arrive —
// the grid is below the fold and the icons are decorative.
...
export const HowItWorks = () => {
	const [icons, setIcons] = useState<Record<string, IconComponent> | null>(
		null,
	);
	...
	useEffect(() => {
		...
		const load = () => {
			import("~/lib/share-targets")
				.then((m) => {
					if (!alive) return;
					setIcons(
						Object.fromEntries(m.WEB_TARGETS.map((t) => [t.name, t.icon])),
					);
				})
				.catch(() => {});
		};
		...
		// The grid sits below the fold — don't spend the LCP window downloading
		// brand icons that aren't visible yet.
		...
		>
			<div className="mx-auto max-w-7xl">
				...
				<p className="pt-7 max-w-2xl text-lg leading-8 text-[#4a5a52]">
					...
					browser activity signals and progressively turns them into a
					structured representation of what happened.
				</p>
				...
				<div className="mt-10 grid overflow-hidden rounded-2xl border-[3px] border-[#102219] bg-lime shadow-hard-xl lg:grid-cols-[1.05fr_0.95fr]">
					...
					<div
						className="grid grid-cols-2 gap-3 border-t-[3px] border-[#102219] p-6 sm:grid-cols-3 sm:p-8 lg:border-l-[3px] lg:border-t-0"
						ref={gridRef}
					>
						{WEB_TARGET_DEFS.map(({ name }) => {
							...
							return (
								...
								>
									{Icon ? (
										...
										// Reserve the icon box so the async icon load can't shift layout.
